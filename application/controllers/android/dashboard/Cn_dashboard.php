...
class Cn_dashboard extends CI_Controller {
 ...
 // print_r($purohit_id);die();
            ...
            $this->db->where('C.pooja_date>=', $curr_date);
           ...
           //echo "<pre>"; print_r($purohit_request_data); die();
            $finalarray=array();
           ...
           // echo "<pre>"; print_r($output); die();
            ...
            if (!empty($output)) {
                ...
                $resultarray = array('error_code' => '1','purohit_request_list'=>$output, 'request_list_count'=>count($purohit_request_list),'message' => 'Request List.');
            } else {
                $resultarray = array(
                    'error_code' => '2',
...
// print_r($output);die();
            if (!empty($output)) {
                ...
                $resultarray = array('error_code' => '1','purohit_today_list'=>$output, 'today_list_count'=>count($purohit_today_list), 'message' => 'Today List.');
            } else {
                $resultarray = array(
                    'error_code' => '2',
      ...
      public function upcoming_list() {
        ...
        if (!empty($checkuser[0]['status']) && $checkuser[0]['status'] == '2') {
            ...
            $this->db->where('C.fk_purohit',$purohit_id);
            $this->db->group_by('pr.fk_pooja_order_id');
            $this->db->order_by('C.pooja_date_time ASC');
            $this->db->limit($limit, $offset);
            ...
            $this->db->where('date(C.pooja_date)>',date('Y-m-d'));
            $this->db->where('C.fk_purohit',$purohit_id);
            // $this->db->order_by('C.pooja_date', 'ASC');
            // $this->db->order_by('C.pooja_time', 'ASC');
            $this->db->group_by('pr.fk_pooja_order_id');
            

           $purohit_upcoming_data_count=$this->db->get()->result_array();


            if (!empty($purohit_upcoming_data)) {

                $resultarray = array('error_code' => '1','purohit_upcoming_list'=>$purohit_upcoming_data, 'upcoming_list_count'=>count($purohit_upcoming_data_count), 'message' => 'Upcoming List.');
            } else {
                $resultarray = array(
                    'error_code' => '2',
       ...
       public function upcoming_list_view() {
        ...
        if (!empty($checkuser[0]['status']) && $checkuser[0]['status'] == '2') {
            ...
            if (!empty($pooja_upcoming_order_view)) {
                ...
                $resultarray = array('error_code' => '1','Pooja_upcoming_order_view'=>$pooja_upcoming_order_view, 'exclusive' => $services_exclusive, 'excl_msg' => $excl_msg, 'profile_image'=>base_url().'upload/customer_profile/', 'message' => 'Pooja details.');
            } else {
                $resultarray = array(
                    'error_code' => '2',
                    'message' => "Pooja details are empty."
    ...
    public function upcoming_list_view_cancel() {
        ...
        if (!empty($cancell_status) && $cancell_status=='4') {
                ...
                if(date($current_time) > date($data_time_for_cancellation)){
                    ...
                    $this->db->from('purohit_customer_pooja_order');
                    ...
                    $this->Md_database->sendPushNotification($resultarray,$target);
                      ...
                      $table = "notifications";
                            ...
                            $this->Md_database->insertData($table, $insert_data);
                                  ...
                                  //fine and balance Store in purohit payment history
                    ...
                    $check_balance=$this->db->get()->result_array();
                    if (!empty($check_balance)){
                        $balance = $check_balance[0]['balance'] - $fine_for_cancel ;                  
                    }else{
                        $balance ='-'.$fine_for_cancel;
                    }
                    ...
                    $insert_data = array(
                        ...
                        'remark' => 'Debited for purohit cancelled Puja ID : SP'.$pooja_order_id,
                        'balance' => $balance,
                ...
                /*[End]:: send message  and mail to customer when order cancelled by purohit*/
                 ...
                 //update fk_purohit empty && pooja_order_status  when cancelled order by purohit
                    ...
                    $update_data['admin_purohit_assign_status'] = '2';
        ...
        if (!empty($checkuser[0]['status']) && $checkuser[0]['status'] == '2') {
            ...
            exit();
                ...
                $this->db->select('B.pk_id as fk_customer_id,o.pk_id as pooja_order_id,pkg.pk_id as pkg_id,o.created_date as order_date,o.pooja_address,o.pooja_area,p.pooja_name,o.pooja_time,o.pooja_date,o.total_pkg_price_exclusive,pkg.description,o.pooja_order_status,o.pooja_city,B.customer_name,B.customer_mobile_no,B.customer_photo,o.reached_date_time,o.reached_otp,o.completed_date_time,o.completed_otp,o.reached_otp_verified, pkg.package, o.package_charges, o.purohit_percentage');
                $this->db->from('customer_pooja_order as o');
                $this->db->join('pooja as p','p.pk_id=o.fk_pooja_id');
                $this->db->join('package as pkg','pkg.pk_id=o.fk_package_id');
                $this->db->join('customer_registration as B', 'B.pk_id=o.fk_user_id');
            ...
            if (!empty($pooja_today_order_view)) {
                ...
                $resultarray = array('error_code' => '1','Pooja_today_order_view'=>$pooja_today_order_view, 'exclusive' => $services_exclusive, 'excl_msg' => $excl_msg, 'profile_image'=>base_url().'upload/customer_profile/', 'message' => 'Pooja details.');
            } else {
                $resultarray = array(
                    'error_code' => '2',
                    'message' => "Pooja details are empty."
        ...
        if (!empty($checkuser[0]['status']) && $checkuser[0]['status'] == '2') {
            ...
            if(!empty($cancell_status) && $cancell_status =='4') {
                ...
                if(date($current_time) > date($data_time_for_cancellation)){
                    ...
                    $this->db->order_by('pk_id','DESC');
                    $check_balance=$this->db->get()->result_array();
                    //echo "<pre>"; print_r($fine_for_cancel); die();
                    if (!empty($check_balance)){
                        $balance = $check_balance[0]['balance'] - $fine_for_cancel ;                  
                    }else{
                        $balance ='-'.$fine_for_cancel;
 ...
 /*Start::Send Request & push notification to purohit only send area wise those user have area*/
                    ...
                    // $update_data['pooja_order_date_plus1']=date("Y-m-d H:i:s", strtotime("+1 hours"));
                    $update_data = array(
                        'pooja_order_status' => 2,
                        'fk_purohit' => NULL,
                        'pooja_order_date_plus1' => date("Y-m-d H:i:s", strtotime("+1 hours")),
                        'admin_assign_purohit_remark' => NULL,
                        'admin_purohit_assign_status' => 2
                        );
      ...
      public function reached_location_otp() {
        ...
        if (!empty($checkuser[0]['status']) && $checkuser[0]['status'] == '2') {
         ...
         /*End::get customer mobile number*/
            ...
            $message = 'Hi, ' . $otp.' is your OTP to confirm the Purohit arrived at your Puja location. Please provide the OTP to purohit';
            $this->Md_database->sendSMS($message, $mobile_no);
    ...
    $otp = !empty($this->input->post('otp'))?$this->input->post('otp'):'';
   // print_r($otp);
    $purohit_id = !empty($this->input->post('purohit_id'))?(int)$this->input->post('purohit_id'):'';
    
   // print_r($purohit_id);
   // die();
        $this->db->select('completed_otp,fk_user_id,pk_id,fk_package_id,total_pkg_price_exclusive');
        $this->db->from('customer_pooja_order'); 
        ...
        $pooja_order_id=!empty($otp_verfied[0]['pk_id'])?$otp_verfied[0]['pk_id']:'';
        $package_id=!empty($otp_verfied[0]['fk_package_id'])?$otp_verfied[0]['fk_package_id']:'';
        $total_pkg_price_exclusive=!empty($otp_verfied[0]['total_pkg_price_exclusive'])?$otp_verfied[0]['total_pkg_price_exclusive']:'';
        // print_r($db_getotp);
        // die();
        ...
        if ($otp != $db_getotp) {
            ...
            // echo "<pre>"; print_r($inserted_data); die();
            $cond = array('completed_otp' => $db_getotp);
            $ret = $this->Md_database->updateData('customer_pooja_order', $inserted_data, $cond);
            $resultarray = array('error_code' => '1','message' =>'OTP verified successfully');

            //Completed puja amount store in payment history of purohit with balance
                // $this->db->select('balance,pk_id');
                // $this->db->from('purohit_package,purohit_percentage'); 
                // $this->db->where('fk_pooja_order_id',$package_id);
                // $this->db->order_by('pk_id','DESC');
                // $check_purohit_amount=$this->db->get()->result_array();

            $this->db->select('purohit_percentage');
            $this->db->from('purohit_customer_pooja_order as A');
            $this->db->where('A.pk_id', $pooja_order_id);
            $purohit_percent_per_puja_data=$this->db->get()->result_array();
            $purohit_percent_per_puja=!empty($purohit_percent_per_puja_data[0]['purohit_percentage'])?$purohit_percent_per_puja_data[0]['purohit_percentage']:'0';

            $admin_commision_percent=100-$purohit_percent_per_puja;
            $admin_comission = $total_pkg_price_exclusive*$admin_commision_percent/100;
          
            $purohit_earnig=$total_pkg_price_exclusive-$admin_comission;
            // print_r($comission);die();

                $this->db->select('balance,pk_id,transaction_type');
                $this->db->from('purohit_purohit_transaction_history'); 
                $this->db->where('fk_purohit_id',$purohit_id);
                $this->db->order_by('pk_id','DESC');
                $check_balance=$this->db->get()->result_array();
                if (!empty($check_balance)){                        
                    $balance = $check_balance[0]['balance'] + $purohit_earnig;                  
                    $amount = $purohit_earnig;                                     
                }else{
                    $amount =$purohit_earnig ;
                    $balance =$purohit_earnig;
                }
                $insert_data['balance'] = $balance;
                $insert_data['fk_pooja_order_id'] = $pooja_order_id;
                $insert_data['fk_purohit_id'] = $purohit_id;
                $insert_data['amount'] = $amount;
                $insert_data['transaction_type'] = '3';
                //$insert_data['remark'] = 'credit amount after completed puja ID: SP'.$pooja_order_id;
                $insert_data['remark'] = 'Amount will be credited in a short while';
                $insert_data['transaction_date'] = date('Y-m-d');
                $insert_data['transaction_time'] = date('H:i:s');
                $res=$this->Md_database->insertData('purohit_purohit_transaction_history',$insert_data);  
