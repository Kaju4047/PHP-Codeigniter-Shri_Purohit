...
class Cn_my_booking extends CI_Controller {
   ...
   // echo "<pre>";		print_r($pooja_listing);die();
        $i=1;
          ...
          $html.='<div class="col-md-6 col-sm-6 col-6">';
          ...
          $html.='</div>';
          $html.='<div class="col-md-6 col-sm-6 col-6">';
          $html.='<div class="order-cont mb-1 float-right text-right">';
              if($value['fk_purohit']=='' && $value['cancelled_by_purohit']=='' && $value['pooja_status']!='3' && $value['pooja_status']!='4' && $value['pooja_status']!='2')
                ...
                elseif($value['pooja_status']=='2')
                    { $setText="Completed";}
                elseif(!empty($value['cancelled_by_purohit']) && $value['pooja_status']!='4')
...
public function order_view($order_id) 
{
   ...
   $customerid = !empty($this->session->userdata('CTMRPKID'))?$this->session->userdata('CTMRPKID'):'';
    ...
    $this->db->select('cpo.pk_id as order_id, cpo.fk_user_id, cpo.fk_pooja_id as puja_id, cpo.fk_package_id as pkg_id, cpo.fk_purohit, cpo.pooja_date, cpo.pooja_time, cpo.pooja_address, cpo.pooja_city, cpo.pooja_area, cpo.total_pkg_price_exclusive, cpo.admin_purohit_assign_status, cpo.pooja_status, cpo.puja_completed_by, cpo.cancellation_charges, cpo.created_date, cpo.pooja_order_status, pp.pooja_name, p.description, prp.first_name, prp.middle_name, prp.last_name, prp.mobile_no, prp.upload_profile_image, prp.city_name, prp.address');
    $this->db->from('customer_pooja_order as cpo');
    $this->db->join('purohit_pooja as pp', 'pp.pk_id = cpo.fk_pooja_id', 'LEFT');
    $this->db->join('package as p', 'p.pk_id = cpo.fk_package_id', 'LEFT');
    $this->db->join('purohit_registered_purohit as prp', 'prp.pk_id = cpo.fk_purohit', 'LEFT');
    $this->db->where('cpo.status', 1);
    $this->db->where('pp.status', 1);
    $this->db->where('cpo.pk_id', $order_id);
  ...
  // echo "<pre>";print_r($order_view_data);die();
    ...
    $this->db->select('psc.services_charges, psc.service_id as fk_services_id, mas.service_name as servicename');
    ...
    $this->db->where('psc.puja_order_id', $value['order_id']);
...
// echo "<pre>";print_r($services_exclusive);die();
  ...
  /*End::customer pooja rating show*/
    ...
    if ($this->form_validation->run() === FALSE) {
        ...
        $pooja_date_time = $pooja_date.''.$pooja_time; 
    ...
    redirect(base_url('front-customer-login'));
        ...
        $update_data = array(
            ...
            'pooja_date_time' => date('Y-m-d H:i:s', strtotime($pooja_date_time))
        );
 ...
 public function rating_action()
     ...
     public function customer_cancel_pooja_order()
    {
     ...
     // echo date('Y-m-d h:i', strtotime("-48 hours"));
      ...
      $this->db->select('fk_purohit,admin_purohit_assign_status,pooja_date,pooja_time,pooja_address,pooja_city,pooja_name,total_pkg_price_exclusive,fk_package_id');
    ...
    $total_pkg_price_exclusive=!empty($accept_assign_purohit[0]["total_pkg_price_exclusive"])?$accept_assign_purohit[0]["total_pkg_price_exclusive"]:'';
    $package_id=!empty($accept_assign_purohit[0]["fk_package_id"])?$accept_assign_purohit[0]["fk_package_id"]:'';
...
if(date($current_time) > date($data_time_for_cancellation)){
    ...
    $this->db->select('cancellation_percentage_purohit');
    $this->db->from('purohit_master_cancellation_percentage_for_purohit');
    $this->db->where('status','1');
    $this->db->limit('1');
    $this->db->order_by('pk_id','desc');
    $cancellation_percent_to_purohit=$this->db->get()->result_array();
     ...
     // cancellation percent given to purohit when customer canclled the puja within 48hr
     $purohit_percent=!empty($cancellation_percent_to_purohit[0]['cancellation_percentage_purohit']) ?$cancellation_percent_to_purohit[0]['cancellation_percentage_purohit']:'';
     $purohit_percenrt_cancel=$puja_pkg_ammount*($purohit_percent/100);
     
     
          //Balance Store in purohit payment history
          $this->db->select('pk_id,purohit_percentage');
          $this->db->from('purohit_package'); 
          $this->db->where('pk_id',$package_id);
          $this->db->order_by('pk_id','DESC');
          $check_purohit_amount=$this->db->get()->result_array();
          // print_r($puja_order_id);
          // print_r($check_purohit_amount);
          // die();
          ...
          $check_balance=$this->db->get()->result_array();
            // $purohit_percentage_amount = $total_pkg_price_exclusive*$check_purohit_amount[0]['purohit_percentage']/100;
          if (!empty($check_balance)){
              $balance = $check_balance[0]['balance'] + $purohit_percenrt_cancel ;      
              $amount=    $purohit_percenrt_cancel  ;      
          }else{
              $balance =$purohit_percenrt_cancel;
              $amount= $purohit_percenrt_cancel  ;      
          }
          ...
          $insert_data = array(
              ...
              'transaction_type'=>'3',
              'amount' => $amount,
              'remark' => 'Credit for purohit for cancelled puja Id : SP'.$puja_order_id,
              'balance' => $balance,
              'status' => '1');
  ...
  // print_r($fine_for_cancel);die();
      ...
      $update_data = array(
                ...
                'cancellation_amount_purohit'=>$purohit_percenrt_cancel,
                'cancellation_type'=>'1',
                );
    ...
    public function purohit_lat_long(){
        ...
        $puja_status= !empty($this->input->get('puja_status')) ? $this->input->get('puja_status') : '';
        $order_id= !empty($this->input->get('order_id')) ? $this->input->get('order_id') : '';
        
        $this->db->select('purohit_latitude,purohit_longitude,pooja_status,B.pk_id');
        $this->db->from('registered_purohit as A');
        $this->db->join('purohit_customer_pooja_order as B', 'B.fk_purohit=A.pk_id');
        $this->db->where('B.fk_purohit',$purohit_id); 
        $this->db->where('B.pk_id',$order_id); 
        $this->db->where('B.reached_otp_verified<>', '1');
        // $this->db->where('B.pooja_status','5'); 
        $this->db->where('A.status',1);
        $purohit_data = $this->db->get()->result_array();
