      ...
      <h1>Refund Puja Booking List
       <!--   <div class="pull-right">
         ...
         <div class="pull-right">
            ...
            <a href="<?php echo base_url(); ?>admin/todays-pooja-booking-list?cust_id=<?php echo (!empty($cust_id) ? $cust_id : ''); ?>&&purohit_id=<?php echo (!empty($purohit_id) ? $purohit_id : ''); ?>"><button type="button" class="btn btn-default">Todays</button></a>
            ...
            <a href="<?php echo base_url(); ?>admin/refund-pooja-booking-list?cust_id=<?php echo (!empty($cust_id) ? $cust_id : ''); ?>&&purohit_id=<?php echo (!empty($purohit_id) ? $purohit_id : ''); ?>"><button type="button" class="btn btn-refund">Refund</button></a>
             ...
             <div class="col-md-2 no-left-pad form-group margin-bottom">
                ...
                <div class="col-md-2 form-group">
                    <label>To Puja Scheduled Date </label>
                      <div class="input-group date" data-date-format="dd.mm.yyyy" >
                      <input type="text"  id="todate" name="filter_todate"  class="form-control" placeholder="dd-mm-yyyy" value="<?php echo !empty($filter_todate) ? date('d-m-Y',strtotime($filter_todate)) : '';?>" autocomplete="off">
                      <div class="input-group-addon">
                        <span class="glyphicon glyphicon-calendar"></span>
                      </div>
                    </div>
                ...
                <div class="col-md-2 form-group">
                 <!--  <button type="button" class="btn btn-warning filter-btn"><i class="fa fa-filter" aria-hidden="true"></i> Filter</button> -->
                  <button type="submit" class="btn btn-primary filter-btn" onclick="javascript: form.action='<?php echo base_url('admin/refund-pooja-booking-list');?>';"><i class="fa fa-filter"></i> Filter</button>
               </div>
                 <div class="col-md-2 form-group">
                  <?php if(!empty($refundedPoojaDeatails)){?>
                   <button type="submit" class="btn btn-primary filter-btn" onclick="javascript: form.action='<?php echo base_url('admin/pooja-booking-export-to-excel/5');?>';">Export to Excel</button>
                 <?php }?>
                </div>
            </div>
            </form>
                ...
                <div class="row">
                  <div class="col-md-6" style="display: inline-block;float: right;margin-bottom: 10px;">
                     <form name="frmSearch" id="frmSearch" method="GET" autocomplete="off">
                        ...
                        <button type="submit" class="btn btn-primary" title="Search" onclick="javascript: form.action='<?php echo base_url('admin/refund-pooja-booking-list');?>';" style="position: absolute;top: 0;right: 0;margin-right: 15px;height: 34px;"><i class="fa fa-search"></i></button>
                     </form>
                  </div>
               </div>
               <table class="table table-bordered table-striped table-hover" width="100%">
                  <thead>
                     <tr>
                        ...
                        <th width="12%">Puja Created Date & Time</th>
                        <th width="12%">Customer Name</th>
                        <th width="9%">Mobile No.</th>
                        <th width="13%">Purohit Name</th>
                        <th width="13%">Puja Scheduled On</th>
                  ...
                  <tbody>
                    <?php
                     ...
                     if (!empty($refundedPoojaDeatails)) {
                           $page_no= !empty($this->uri->segment(3)) ? $this->uri->segment(3): 1;
                           $i = ($page_no * 10) - 9;
                      foreach ($refundedPoojaDeatails as $key => $value){
                                            
                     ?>
                     <tr>
                        <td class="text-center"><?= $i;?></td>
                        <td>SP<?php echo !empty($value['fk_pooja_order_id'])?ucfirst($value['fk_pooja_order_id']):''; ?></td>
                        ...
                        <!-- <td><?php echo !empty($value['refund_amount'])?$value['refund_amount']:''; ?></td> -->
                        <td><?php if (!empty($value['refund_status']) && $value['refund_status'] == 'Not Refund') {?>
                          <span class="cl-green"><?php
                               echo "New"; ?></span><?php
                            }elseif(!empty($value['refund_status']) && $value['refund_status'] == 'Refund'){
                              ?> <span class="cl-blue"><?php
                              echo "Refunded";
                            }
                         ?></span></td>
                        <?php if(in_array('pooja_booking_view', $privilige)){ ?>
                        <td class="text-center">
                          <a href="<?php echo base_url(); ?>admin/view-refund-pooja-booking/<?php echo (!empty($value['fk_pooja_order_id']) ? $value['fk_pooja_order_id'] : ''); ?>"><button type="button" class="btn btn-primary btn-xs" title="View"><i class="fa fa-eye"></i></button></a>
                        </td>
                        <?php }?>
                     </tr>
                       <?php
