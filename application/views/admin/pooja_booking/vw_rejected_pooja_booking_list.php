   ...
   <section class="content-header">
      <h1>Rejected Puja Booking List
        <!--  <div class="pull-right">
            ...
            <a href="<?php echo base_url(); ?>admin/reject-pooja-booking-list"><button type="button" class="btn btn-warning">Rejected</button></a>
         ...
         <div class="pull-right">
            ...
            <a href="<?php echo base_url(); ?>admin/reject-pooja-booking-list?cust_id=<?php echo (!empty($cust_id) ? $cust_id : ''); ?>&&purohit_id=<?php echo (!empty($purohit_id) ? $purohit_id : ''); ?>"><button type="button" class="btn btn-warning">Rejected</button></a>
              ...
              <form id="filter"  method='get'  enctype="multipart/form-data">
                ...
                <div class="col-md-2 form-group">
                  <!-- <button type="button" class="btn btn-warning filter-btn"><i class="fa fa-filter" aria-hidden="true"></i> Filter</button> -->
                   <button type="submit" class="btn btn-primary filter-btn" onclick="javascript: form.action='<?php echo base_url('admin/reject-pooja-booking-list');?>';"><i class="fa fa-filter"></i> Filter</button>
               </div>
                <div class="col-md-2 form-group">
                  <?php if(!empty($rejectedPoojaDeatails)){?>
                   <button type="submit" class="btn btn-primary filter-btn" onclick="javascript: form.action='<?php echo base_url('admin/pooja-booking-export-to-excel/6');?>';">Export to Excel</button>
                 <?php }?>
                </div>
              </form>
            </div>
            ...
            <div class="box-body">
               <div class="row">
                  <div class="col-md-6" style="display: inline-block;float: right;margin-bottom: 10px;">
                     <form name="frmSearch" id="frmSearch" method="GET" autocomplete="off">
                        ...
                        <button type="submit" class="btn btn-primary" title="Search" onclick="javascript: form.action='<?php echo base_url('admin/reject-pooja-booking-list');?>';" style="position: absolute;top: 0;right: 0;margin-right: 15px;height: 34px;"><i class="fa fa-search"></i></button>
                     </form>
                  </div>
               </div>
               <table id="example" class="table table-bordered table-striped table-hover" width="100%">
                  ...
                  <tbody>
                     ...
                     if (!empty($rejectedPoojaDeatails)) {
                           $page_no= !empty($this->uri->segment(3)) ? $this->uri->segment(3): 1;
                           $i = ($page_no * 10) - 9;
                      foreach ($rejectedPoojaDeatails as $key => $value){                                            
                     ?>
                     <tr>
                       ...
                       <!--  <td><?php echo !empty($value['purohit_name'])?ucfirst($value['purohit_name']):'-'; ?></td> -->
                        ...
                        <td class="text-center">
                          ...
                          <a href="<?php echo base_url(); ?>admin/view-rejected-pooja-booking/<?php echo (!empty($value['pk_id']) ? $value['pk_id'] : ''); ?>"><button type="button" class="btn btn-primary btn-xs" title="View"><i class="fa fa-eye"></i></button></a>
                          <?php } ?>
                          <?php if(in_array('pooja_booking_delete', $privilige) ){ ?>
                          <a href="<?php echo base_url(); ?>admin/delete-rejected-pooja-booking/<?php echo (!empty($value['pk_id']) ? $value['pk_id'] : ''); ?>" onClick="return confirm('Are you sure you want to delete record?')"><button type="button" class="btn btn-danger btn-xs" title="Delete"><i class="fa fa-trash"></i></button></a> 
                           <?php } ?>
                        </td>
                         <?php } ?>
                     </tr>
