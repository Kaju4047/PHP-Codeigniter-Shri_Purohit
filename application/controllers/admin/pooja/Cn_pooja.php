...
class Cn_pooja extends CI_Controller {
    ...
    public function addPooja() {
        ...
        $categoryDetails = $this->Md_database->getData($table, $select, $condition, 'category ASC', '');        
    ...
    public function addPoojaAction(){
        ...
        $this->form_validation->set_rules('silent_feature', 'Salient Feature', 'required|trim|max_length[1200]');        
        ...
        // print_r($photoDoc);die();
        //     if (!empty($this->input->post('fileold'))) {
        //        unlink(FCPATH . 'upload/admin/pooja/";' . $this->input->post('fileold'));
        //     }
        // }
           if (!empty($_FILES['pooja_image']['name'])) {
                        $rename_name = uniqid(); //get file extension:
                        ...
                        $upload_type = "jpg|jpeg|png|JPG|JPEG|PNG";
                        $photoDoc = $this->Md_database->uploadFile($path, $upload_type, "pooja_image", "", $newname);   
                        // $inserted_data['cv']=$photoDoc ;                        
                        // print_r($photoDoc);die();
                    }  
        if (empty($txtid)) {
            $table = "pooja";
            ...
            $this->session->set_flashdata('success', 'Puja has been inserted successfully.');
        }else{
            // update data code
            $table = "pooja";
            $update_data = array(
            ...
            if (!empty($photoDoc)) {
                $update_data['pooja_image']=$photoDoc;
            }
            $update_id = $this->Md_database->updateData($table, $update_data, $condition);      
            $this->session->set_flashdata('success', 'Puja has been updated successfully.');
        }           
    ...
    public function deletePooja($id){
        ...
        if (!empty($ret)) {
            $this->session->set_flashdata('success', "Puja details has been deleted successfully.");
            redirect($_SERVER['HTTP_REFERER']);    
       }
    }  
}
